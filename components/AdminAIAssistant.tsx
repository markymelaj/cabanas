  ...
  async function analyze() {
    ...
    try {
      const res = await fetch('/api/admin/ai/conflict-advice', {
        ...
        body: JSON.stringify({ reservationId, salonQuoteId }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'No se pudo analizar la reserva.')
      setAdvice(data.advice)
    } catch (err: any) {
      setError(err.message || 'No se pudo analizar la reserva.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="rounded-lg border border-lago-100 bg-white p-5 print:hidden">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className="font-display text-xl text-lago-900">Decisión operativa</h3>
        </div>
      ...
      <button onClick={analyze} disabled={loading} className="btn-primary mt-4 w-full text-sm disabled:opacity-50">
        {loading ? <><Loader2 size={16} className="animate-spin" />Analizando</> : <><Sparkles size={16} />Analizar con Claude</>}
      </button>
        ...
        <div className="mt-5 space-y-4">
          ...
          <div>
            <div className="mb-2 flex items-center justify-between gap-3">
              <p className="text-xs uppercase tracking-[0.14em] text-volcan-400">WhatsApp sugerido</p>
