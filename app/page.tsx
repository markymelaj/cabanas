    ...
    <>
      ...
      <main>
        <section className="relative overflow-hidden bg-lago-950 text-white">
          ...
          />
          <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(17,30,24,0.98)_0%,rgba(17,30,24,0.92)_48%,rgba(17,30,24,0.62)_100%)]" />
          <div className="absolute inset-0 bg-[linear-gradient(to_bottom,rgba(20,30,25,0.08),rgba(20,30,25,0.88))]" />

          <div className="relative mx-auto grid min-h-[650px] max-w-7xl items-center gap-12 px-6 pb-16 pt-28 sm:px-8 sm:pb-20 sm:pt-32 lg:grid-cols-[1.08fr_0.92fr] lg:px-12 lg:pb-24 lg:pt-36">
            <div className="max-w-[680px]">
              <p className="eyebrow eyebrow-dark mb-6">Reservas sin comisión · Implementación personalizada</p>
              <h1 className="font-display text-[46px] leading-[0.98] tracking-[-0.042em] text-white sm:text-[56px] lg:text-[62px]">
                <span className="block">Menos mensajes sueltos.</span>
                <span className="mt-1 block text-arena-100">Más reservas bajo control.</span>
              </h1>
              <p className="mt-7 max-w-[610px] text-[17px] leading-8 text-lago-100 sm:text-lg">
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <Link href="/cabanas" className="btn-primary min-h-12 px-6">Probar experiencia huésped <ArrowRight size={16} /></Link>
                <Link href="/admin" className="btn-outline min-h-12 border-white/20 bg-white/5 px-6 text-white hover:bg-white/10">Entrar al panel demo</Link>
              </div>
              <div className="mt-7 flex flex-wrap gap-x-6 gap-y-2 text-sm text-lago-200">
...
function DashboardPreview() {
  return (
    <div className="relative mx-auto w-full max-w-[510px] lg:justify-self-end">
      <div className="absolute -inset-5 rounded-[2rem] bg-arena-300/10 blur-2xl" />
      <div className="relative overflow-hidden rounded-[1.65rem] border border-white/20 bg-white/[0.97] p-2.5 text-lago-950 shadow-[0_28px_80px_rgba(4,12,8,0.30)]">
        ...
        <div className="mx-3 mb-3 overflow-hidden rounded-xl border border-arena-100">
          <div className="flex items-center justify-between bg-arena-50 px-4 py-3">
            <span className="text-[11px] font-bold uppercase tracking-wide text-volcan-500">Próximas estadías</span>
            <CalendarCheck size={16} className="text-lago-600" />
          </div>
          {[
            ['Carolina Muñoz', 'Cabaña Bosque · 14–17 jul'],
            ['Matías Rojas', 'Cabaña Familiar · 18–20 jul'],
            ['Paula Soto', 'Cabaña Bosque · 21–24 jul'],
          ].map(([name, detail], i) => (
            <div key={name} className="flex items-center justify-between gap-3 border-t border-arena-100 px-4 py-3">
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold">{name}</p>
                <p className="truncate text-xs text-volcan-500">{detail}</p>
              </div>
              <span className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-bold ${i === 0 ? 'bg-amber-100 text-amber-800' : 'bg-green-100 text-green-800'}`}>
                {i === 0 ? 'PENDIENTE' : 'CONFIRMADA'}
              </span>
            </div>
          ))}
        </div>
...
function PreviewMetric({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl border border-arena-100 bg-arena-50 p-3.5"><p className="text-[10px] font-bold uppercase tracking-wide text-volcan-500">{label}</p><p className="mt-2 font-display text-2xl">{value}</p></div>
}
